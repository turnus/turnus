 ...
 * 
 */
package turnus.analysis.partitioning.tabusearch.test;
 ...
 * 
 */
public class TabuSearchTEST extends Analysis<ActorStatisticsReport> {
	...
	private long seed = 181014;//System.currentTimeMillis();
	...
	public TabuSearchTEST(TraceProject tProject, TraceWeighter tWeighter, BufferSize bufferSize) {
		super(tProject);
	...
	public ActorStatisticsReport run() throws TurnusException {
		...
		if (!improvedOverall) {
			...
			File output = new File(configuration.getValue(MAPPING_FILE).getParent() + "/" + System.currentTimeMillis() + "-" + configuration.getValue(TABU_GENERATOR) + ".txt");
			PrintWriter out;
			try {
				out = new PrintWriter(new FileWriter(output));
				out.print("Last improvement made after " + (lastImprovementTime - startTime) / 1000 / 60 + " minutes");
				out.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
