 ...
 * 
 */
public class TabuSearchTimeTuningPortioned extends Analysis<Integer> {
	...
	private int allowedNoImprovement = 5; // (in minutes) how long are we going to wait for the next improvement
	
	public TabuSearchTimeTuningPortioned(TraceProject tProject, TraceWeighter tWeighter, BufferSize bufferSize) {
		super(tProject);
	...
	@Override
	public Integer run() throws TurnusException {
		...
		lastImprovementTime = System.currentTimeMillis(); // lastImprovementTimeInitialization
		...
		int timeLimit =  (int)((endTime - startTime) / 1000 / 60);
	...
	private boolean ts(int iteration) {
		...
		long minutesSinceImprovement = (System.currentTimeMillis() - lastImprovementTime) / 1000 / 60;
		if (minutesSinceImprovement >= allowedNoImprovement) {
			endTime = lastImprovementTime; // break the algorithm if the time rule is violated
			return false;
			...
			while (ccontinue) {
				if (random() <= p) {
					...
					if (fsCand < 0.99 * fStar) // make sure that the improvement will be taken into consideration if we reach the time limit with this neighborhood
				...
				minutesSinceImprovement = (System.currentTimeMillis() - lastImprovementTime) / 1000 / 60;
				if (minutesSinceImprovement >= allowedNoImprovement) {
					endTime = lastImprovementTime; // break the algorithm if the time rule is violated
					return false;
			...
			if (bestf < fStar) {
				...
				sStar = sNeighbor;
				// improvement by at least 1%
				if (bestf < 0.99 * fStar) // previous check was temporary and could have been overwritten
					lastImprovementTime = System.currentTimeMillis();
			}
			sCurrent = sNeighbor;
