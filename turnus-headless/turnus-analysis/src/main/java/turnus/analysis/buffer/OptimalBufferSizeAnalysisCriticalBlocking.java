 ...
 */
public class OptimalBufferSizeAnalysisCriticalBlocking extends Analysis<OptimalBuffersReport> {
	
	/**
	 * Rank buffers by critical blockings / all blockings.
	 */
	...
	public OptimalBufferSizeAnalysisCriticalBlocking(TraceProject project) {
		super(project);
	...
	public OptimalBuffersReport run() throws TurnusException {
		...
		for (;;) {
			...
			Map<Buffer, Double> criticalBlockings = getCriticalBlockings(pcpData, bbData);	  // for holding the values
			...
			for (Buffer b : criticalBuffers) {
				System.out.println(b + ", ratio: " + criticalBlockings.get(b));
			}
			...
			for (Buffer b : otherBuffers) {
				System.out.println(b + ", blockings: " + otherBlockings.get(b));
			}
	...
	private Map<Buffer, Double> getCriticalBlockings(BottlenecksWithSchedulingReport pcpReport, BufferBlockingReport bbReport) {
		Map<Buffer, Double> result = new HashMap<>();
		
		for (ActionBottlenecksWithSchedulingData data : pcpReport.getActionsData()) {
			Map<Buffer, Integer> bb = data.getBlockingInstances();
			for (Entry<Buffer, Integer> b : bb.entrySet()) {
				double value = result.containsKey(b.getKey()) ? result.get(b.getKey()) : 0;
				result.put(b.getKey(), (double) b.getValue() + value);
			}
		}
		
		for (Buffer b : result.keySet()) {
			double ratio = (double) result.get(b) / (double) bbReport.getBlockingInstances().get(b);
			result.put(b, ratio);
		}
	...
	private Map<Buffer, Double> getOtherBlockings(BufferBlockingReport bbReport, Map<Buffer, Double> criticalBlockings) {
		...
		for (Buffer b : nonCriticalBuffers) {
			if (bbReport.getBlockingInstances().keySet().contains(b))
				result.put(b, (double)bbReport.getBlockingInstances().get(b));
		}
