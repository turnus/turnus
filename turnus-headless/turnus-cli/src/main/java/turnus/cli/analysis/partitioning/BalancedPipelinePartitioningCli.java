...
/**
 * CLI command for Balanced Pipeline Partitioning.
 */
@Command(
    name = "balanced-pipeline",
    description = "Run balanced pipeline partitioning",
    mixinStandardHelpOptions = true
)
public class BalancedPipelinePartitioningCli implements PartitioningAlgorithmCli {
    ...
    @Override
    public Integer call() {
        Logger.info("Starting Balanced Pipeline Partitioning...");
        ...
        try {
            ...
            // Run analysis
            BalancedPipelinePartitioning analysis = new BalancedPipelinePartitioning(project, weighter);
            analysis.setConfiguration(config);
            BalancedPipelinePartitioningReport report = analysis.run();
            ...
            // Save report
            File reportFile = new File(outDir, FileUtils.changeExtension(traceFile, "balanced_pipeline_partitioning").getName());
