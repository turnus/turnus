...
import turnus.analysis.buffer.OptimalBufferSizeAnalysisHeavyBlocking;
...
@Command(
	name = "optimal-buffer-heavy-blocking",
	description = "Run optimal buffer size analysis (rank buffers by blocked tokens * time)",
	mixinStandardHelpOptions = true
)
public class OptimalBufferHeavyBlockingCommand implements Callable<Integer> {
	...
	@Override
	public Integer call() {
		Logger.info("Starting Optimal Buffer Size Analysis (HeavyBlocking)...");
		...
		try {
			...
			OptimalBufferSizeAnalysisHeavyBlocking analysis = new OptimalBufferSizeAnalysisHeavyBlocking(project);
			...
			File reportFile = new File(outDir, FileUtils.changeExtension(traceFile, "optimalbuffers-heavy-blocking").getName());
