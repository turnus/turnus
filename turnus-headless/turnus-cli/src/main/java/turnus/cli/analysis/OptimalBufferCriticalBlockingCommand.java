...
import turnus.analysis.buffer.OptimalBufferSizeAnalysisCriticalBlocking;
...
@Command(
	name = "optimal-buffer-critical-blocking",
	description = "Run optimal buffer size analysis (rank buffers by critical blockings ratio)",
	mixinStandardHelpOptions = true
)
public class OptimalBufferCriticalBlockingCommand implements Callable<Integer> {
	...
	@Override
	public Integer call() {
		Logger.info("Starting Optimal Buffer Size Analysis (CriticalBlocking)...");
		...
		try {
			...
			OptimalBufferSizeAnalysisCriticalBlocking analysis = new OptimalBufferSizeAnalysisCriticalBlocking(project);
			...
			File reportFile = new File(outDir,
					FileUtils.changeExtension(traceFile, "optimalbuffers-critical-blocking").getName());
