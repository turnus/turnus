...
import turnus.analysis.partitioning.HypergraphPartitioning;
...
/**
 * CLI command for Hypergraph Partitioning.
 */
@Command(
    name = "hypergraph",
    description = "Run hypergraph partitioning (using external tools like khmetis)",
    mixinStandardHelpOptions = true
)
public class HypergraphPartitioningCli implements PartitioningAlgorithmCli {
    ...
    @Option(names = {"--tool"}, description = "External partitioning tool (e.g., khmetis)", defaultValue = "khmetis")
    private String externalTool;

    @Override
    public Integer call() {
        Logger.info("Starting Hypergraph Partitioning...");
        ...
        try {
            ...
            config.setValue(EXTERNAL_PARTITIONING_TOOL, externalTool);
            
            // Run analysis
            HypergraphPartitioning analysis = new HypergraphPartitioning(project, weighter);
            ...
            // Save report
            File reportFile = new File(outDir, FileUtils.changeExtension(traceFile, "hypergraph_partitioning").getName());
