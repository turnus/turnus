...
/**
 * CLI command for Tabu Search Partitioning.
 */
@Command(
    name = "tabu-search",
    description = "Run tabu search partitioning",
    mixinStandardHelpOptions = true
)
public class TabuSearchCli implements PartitioningAlgorithmCli {
    ...
    @Option(names = {"--generator"}, description = "Neighborhood generator (RANDOM, IDLE, COMM_FREQ, BALANCING)", defaultValue = "IDLE")
    private String generator;

    @Override
    public Integer call() {
        Logger.info("Starting Tabu Search Partitioning...");
        ...
        try {
            ...
            // Run Tabu Search
            TabuSearch algorithm = new TabuSearch(project, weighter, null, null, null, false);
            algorithm.setConfiguration(config);
            algorithm.setGenerator(generator);
            ...
            // Save report
            File reportFile = new File(outDir, FileUtils.changeExtension(traceFile, "tabu_search_partitioning").getName());
