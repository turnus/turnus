 ...
 *
 */
public class FIFOPartition extends AtomicActorPartition {

	private List<Actor> actorsToExecute = new ArrayList<>();

	public FIFOPartition(List<Actor> actors, String partitionId, int processingElements) {
	...
	public List<Actor> getSchedulables() {
		...
		if (!schedulableActors.isEmpty()) {
			...
			while ((actorsToExecute.size() < schedulableActors.size()) && (actorsToExecute.size() < processingElements())) {
				actorsToExecute.add(schedulableActors.get(index));
				index++;				
			}
		}
