...
import turnus.analysis.profiling.LatencyAnalysisCli;
 ...
 * author Endri Bezati
 */
public class LatencyAnalysisWizard extends Wizard implements IWorkbenchWizard {
	...
	private class OptionsPage extends AbstractWizardPage {
		...
		private WidgetText wSourceActorAction;
		private WidgetText wTargetActorAction;

		protected OptionsPage() {
			super("Latency and Throughput analysis");
			setTitle("Latency and Throughput analysis");
			setDescription("Latency and Throughput analysis");
		}
		...
		protected void createWidgets(Composite container) {
			...
			wSourceActorAction = new WidgetText("Source Actor:Action", "Separate with semicolon actor and action", "",
					container);
			addWidget(wSourceActorAction);
			
			wTargetActorAction = new WidgetText("Target Actor:Action", "Separate with semicolon actor and action, comma for multiple ones", "",
					container);
			addWidget(wTargetActorAction);
		...
		public String getSourceActorAction() {
			return wSourceActorAction.getValue();
		}
		
		public List<String> getTargetActorAction() {
			return Arrays.stream(wTargetActorAction.getValue().split(","))
	...
	public boolean performFinish() {
		...
		configuration.setValue(LATENCY_SOURCE, optionsPage.getSourceActorAction());
		configuration.setValue(LATENCY_TARGETS, optionsPage.getTargetActorAction());
		
		final Job job = new Job("Latency and Throughput analysis") {

			@Override
			protected IStatus run(IProgressMonitor monitor) {
				try {
					new LatencyAnalysisCli().start(configuration, monitor);
