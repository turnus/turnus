...
import turnus.ui.widget.IWidget;
 ...
 *
 */
public interface ILaunchWidget<T> extends IWidget<T>{
	...
	public class Util{
		...
		/**
		 * Perform the apply method
		 * 
		 * @param configuration
		 */
		public static void performApply(ILaunchWidget<?> widget, ILaunchConfigurationWorkingCopy configuration) {
			if (widget.isEnabled()) {
				String value = widget.getValueAsString();
				if (value != null) {
					configuration.setAttribute(widget.getId(), value);
				} else {
					configuration.removeAttribute(widget.getId());
				}
			} else {
				configuration.removeAttribute(widget.getId());
			}
	...
	void performApply(ILaunchConfigurationWorkingCopy configuration) throws CoreException;
