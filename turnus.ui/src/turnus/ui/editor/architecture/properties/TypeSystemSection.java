 ...
 *
 */
public class TypeSystemSection extends AbstractGridBasedSection {

	private final BiMap<EType, Button> typesWidget = HashBiMap.create();
	...
	protected void readValuesFromModels() {
		...
		List<EType> types = pu.getTypes();
		for (Entry<EType, Button> et : typesWidget.entrySet()) {
			Button widget = et.getValue();
			EType type = et.getKey();
			widget.setSelection(types.contains(type));
		}
	...
	protected void writeValuesToModel(Widget widget) {
		...
		EType type = typesWidget.inverse().get(button);
		if (button.getSelection() && !pu.getTypes().contains(type)) {
			pu.getTypes().add(type);
		} else {
			pu.getTypes().remove(type);
		}
