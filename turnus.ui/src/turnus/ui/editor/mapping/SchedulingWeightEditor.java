...
import static turnus.common.TurnusExtensions.SCHEDULE_WEIGHT;
...
/**
 * This class contains the {@link EditorPart} for the {@link SchedulingWeight}
 * file.
 * 
 * @author Endri Bezati
 *
 */
public class SchedulingWeightEditor extends EditorPart {
	...
	private class CellModifier implements ICellModifier {
		...
		public boolean canModify(Object obj, String property) {
			return obj instanceof Triplet && !property.equals(columnNames[ACTOR_ACTION]);
		}
		...
		public Object getValue(Object obj, String property) {
			...
			if (obj instanceof String) {
				...
				Triplet<String, String, String> triplet = (Triplet<String, String, String>) obj;
				switch (columnIndex) {
				case ACTOR_ACTION: {
					result = triplet.v2;
					break;
				...
				case CLOCK_MEAN: {
					ClockCycles w = weights.getWeight(triplet.v1, triplet.v3, triplet.v2);
		...
		public void modify(Object element, String property, Object value) {
			...
			if (columnIndex != ACTOR_ACTION) {
				...
				if (intString.matches("[+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?")) {
					...
					@SuppressWarnings("unchecked")
					Triplet<String, String, String> triplet = (Triplet<String, String, String>) item.getData();
					ClockCycles w = weights.getWeight(triplet.v1, triplet.v3, triplet.v2);
	...
	private class ContentProvider implements ITreeContentProvider {
		...
		public Object[] getChildren(Object obj) {
			if (obj instanceof SchedulingWeight) {
				...
				for (String action : weights.asTable().row(actor).keySet()) {
					for(Map<String, ClockCycles> m : weights.asTable().row(actor).values()) {
						for(String lastAction : m.keySet()) {
							triplets.add(Triplet.create(actor, action, lastAction));
						}
					}
				}
		...
		public Object getParent(Object obj) {
			if (obj instanceof String) {
				...
				return ((Triplet<String, String, String>) obj).v1;
			}
			return null;
		}
		...
		public Object[] getElements(Object input) {
			if (input instanceof SchedulingWeight) {
				return weights.asTable().rowKeySet().toArray();
	...
	private SchedulingWeight weights;
	private boolean isDirty = false;

	private final int ACTOR_ACTION = 0;
	private final int SRC_ACTION = 1;
	private final int CLOCK_MEAN = 2;
	private final int CLOCK_MIN = 3;
	private final int CLOCK_MAX = 4;

	private final String[] columnNames = new String[] { "Actor, Action", " Source Action ", "CC (mean)", "CC (min)",
			"CC (max)" };
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			if (input instanceof FileEditorInput) {
				...
				if (FileUtils.getExtension(file).equals(SCHEDULE_WEIGHT)) {
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		try {
			if (input instanceof FileEditorInput) {
				File file = FileUtils.getFile(((FileEditorInput) input).getFile());
				if (FileUtils.getExtension(file).equals(SCHEDULE_WEIGHT)) {
					weights = new XmlSchedulingWeightReader().load(file);
	...
	public void createPartControl(Composite parent) {
		...
		column.setText(columnNames[SRC_ACTION]);
		column.setWidth(200);
		column.addSelectionListener(getSelectionAdapter(column, SRC_ACTION));
