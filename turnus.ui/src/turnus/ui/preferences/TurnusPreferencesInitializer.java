 ...
 *
 */
public class TurnusPreferencesInitializer extends AbstractPreferenceInitializer {

	public TurnusPreferencesInitializer() {

	}

	@Override
	public void initializeDefaultPreferences() {
		// NOTE for each option remember to register an handler in the
		// Activator#registerPropertyChangeListeners()
		IPreferenceStore store = Activator.getDefault().getPreferenceStore();
		store.setDefault(CONFIG_MAX_THREADS.longName(), DEFAULT_MAX_THREADS);
		store.setDefault(CONFIG_VERBOSE.longName(), false);
		store.setDefault(CONFIG_STREAM_BUFFERS_SIZE.longName(), DEFAULT_STREAM_BUFFER_SIZE);
		store.setDefault(CONFIG_QUEUES_SIZE.longName(), DEFAULT_QUEUE_SIZE);
		store.setDefault(CONFIG_REMOVE_TEMP_TRACE_FILES.longName(), DEFAULT_REMOVE_TEMP_TRACE_FILES);
		store.setDefault(CONFIG_MINIMIZE_MEMORY_USAGE.longName(), DEFAULT_MINIMIZE_MEMORY_USAGE);
	}
