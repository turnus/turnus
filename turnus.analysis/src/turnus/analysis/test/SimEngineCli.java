...
import turnus.adevs.logging.impl.ActionExecutionDebugger;
	...
	private PostProcessingReport run() throws TurnusException {
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				...
				simulation.addDataCollector(new ActionExecutionDebugger());
				//simulation.addDataCollector(actionStatsCollector);
				//simulation.addDataCollector(schedulingCheckCollector);
				
				CriticalPathCollector cpCollector = new CriticalPathCollector(tProject.getNetwork(), partitioning);
				simulation.addDataCollector(cpCollector);
