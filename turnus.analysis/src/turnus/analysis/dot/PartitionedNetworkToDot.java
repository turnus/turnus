...
public class PartitionedNetworkToDot extends NetworkToDot {
	...
	public PartitionedNetworkToDot(Network network, NetworkPartitioning partitioning) {
		super(network);
		partitionMap = new HashMap<>();
		List<String> partitionNames = new ArrayList<>(partitioning.asPartitionActorsMap().keySet());

		for (String actorName : partitioning.asActorPartitionMap().keySet()) {
			int index = partitionNames.indexOf(partitioning.asActorPartitionMap().get(actorName));

			partitionMap.put(actorName, indexcolors[index]);
		}
	}
	...
	protected void instanceColor(String actorName) {
		emitter.emit("<tr><td bgcolor=\"%s\"><font point-size=\"30\" color=\"#ffffff\"> %s </font></td></tr>",
				partitionMap.get(actorName), actorName);
	}
