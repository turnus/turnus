...
public class ScheduledPartialCriticalPathAnalysisCli implements IApplication {
	...
	private ScheduledPartialCriticalPathAnalysis analysis;

	public static void main(String[] args) {
		ModelsRegister.init();

		ScheduledPartialCriticalPathAnalysisCli cliApp = null;

		try {
			cliApp = new ScheduledPartialCriticalPathAnalysisCli();
			cliApp.parse(args);
	...
	private void run() throws Exception {
		monitor.beginTask("Scheduled Partial Critical Path Analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		BottlenecksWithSchedulingReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new ScheduledPartialCriticalPathAnalysis(project);
