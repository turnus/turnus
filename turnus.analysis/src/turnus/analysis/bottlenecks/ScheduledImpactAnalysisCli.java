 ...
 *
 */
public class ScheduledImpactAnalysisCli implements IApplication {

	private Configuration configuration;

	private IProgressMonitor monitor = new NullProgressMonitor();

	private ScheduledImpactAnalysis analysis;

	public static void main(String[] args) {
		ModelsRegister.init();

		ScheduledImpactAnalysisCli cliApp = null;

		try {
			cliApp = new ScheduledImpactAnalysisCli();
			cliApp.parse(args);
	...
	private void run() throws Exception {
		...
		ScheduledImpactAnalysisReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new ScheduledImpactAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath, TurnusExtensions.SCHEDULED_IMPACT_ANALYSIS_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Scheduled impact analysis report stored in \"%s\"", reportFile);

				File cpFile = changeExtension(reportFile, TurnusExtensions.SCHEDULED_BOTTLENECKS_REPORT);
				EcoreUtils.storeEObject(report.getInitialBottlenecksWithScheduling(), project.getResourceSet(), cpFile);
				Logger.info("Initial bottlenecks with scheduling analysis report stored in \"%s\"", cpFile);
