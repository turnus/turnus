...
public class AlgorithmicPartialCriticalPathAnalysisCli implements IApplication {
	...
	private AlgorithmicPartialCriticalPathAnalysis analysis;

	public static void main(String[] args) {
		ModelsRegister.init();

		AlgorithmicPartialCriticalPathAnalysisCli cliApp = null;

		try {
			cliApp = new AlgorithmicPartialCriticalPathAnalysisCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		monitor.beginTask("Algorithmic bottleneck analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		BottlenecksReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new AlgorithmicPartialCriticalPathAnalysis(project);
