 ...
 *
 */
public class IntraActionCommunicationAnalysisCli implements IApplication{
	
	private Configuration configuration;

	private IProgressMonitor monitor = new NullProgressMonitor();

	private IntraActionCommunicationAnalysis analysis;
	
	public static void main(String[] args) {
		ModelsRegister.init();

		IntraActionCommunicationAnalysisCli cliApp = null;

		try {
			cliApp = new IntraActionCommunicationAnalysisCli();
			cliApp.parse(args);
	...
	private void run() throws Exception {
		monitor.beginTask("Intra-Action Communication Analysis", IProgressMonitor.UNKNOWN);
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new IntraActionCommunicationAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath, TurnusExtensions.INTRA_ACTION_COMM_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Intra-Action communication analysis report stored in \"%s\"", reportFile);
