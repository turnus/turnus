 ...
 * 
 */
public class BalancedPipelinePartitioningCli implements IApplication {
	private Configuration configuration;
	private BalancedPipelinePartitioning analysis;
	private IProgressMonitor monitor = new NullProgressMonitor();
	
	public static void main(String[] args) {
		ModelsRegister.init();

		BalancedPipelinePartitioningCli cliApp = null;

		try {
			cliApp = new BalancedPipelinePartitioningCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		monitor.beginTask("Balanced pipeline partitioning analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		TraceWeighter weighter = null;
		String scheduling = null;
		
		BalancedPipelinePartitioningReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new BalancedPipelinePartitioning(project, weighter);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath, TurnusExtensions.BALANCED_PIPELINE_PARTITIONING_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Balanced pipeline partitioning report stored in \"%s\"", reportFile);
