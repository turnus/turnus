 ...
 */
public class WorkloadBalancePartitioningCli implements IApplication {

	private Configuration configuration;
	private WorkloadBalancePartitioning analysis;
	private IProgressMonitor monitor = new NullProgressMonitor();

	public static void main(String[] args) {
		ModelsRegister.init();

		WorkloadBalancePartitioningCli cliApp = null;

		try {
			cliApp = new WorkloadBalancePartitioningCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		monitor.beginTask("Workload balance partitioning analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		TraceWeighter weighter = null;
		String scheduling = null;

		WorkloadBalancePartitioningReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new WorkloadBalancePartitioning(project, weighter);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath,
						TurnusExtensions.WORKLOAD_BALANCE_PARTITIONING_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Workload balance partitioning report stored in \"%s\"", reportFile);
