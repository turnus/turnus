 ...
 * 
 */
public class TabuSearchPROBCli implements IApplication {
	
	private Configuration configuration;
	private TabuSearchPROB tabuSearch;
	private IProgressMonitor monitor = new NullProgressMonitor();
	
	public static void main(String[] args) {
		ModelsRegister.init();

		TabuSearchPROBCli cliApp = null;

		try {
			cliApp = new TabuSearchPROBCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				tabuSearch = new TabuSearchPROB(project, weighter, communication, schedWeight, bufferSize, configuration.getValue(RELEASE_BUFFERS_AFTER_PROCESSING));
