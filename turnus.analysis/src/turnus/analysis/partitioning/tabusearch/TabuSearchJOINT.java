 ...
 * 
 */
public class TabuSearchJOINT extends TabuSearchAbstract {
	...
	public TabuSearchJOINT(TraceProject tProject, TraceWeighter tWeighter, CommunicationWeight commWeight, SchedulingWeight schedulingWeight, BufferSize bufferSize, boolean release) {
		super(tProject);
		...
		movesGenerators.add(new RandomMovesGenerator(project));
		movesGenerators.add(new IdleMovesGenerator(project));
		movesGenerators.add(new CommFreqMovesGenerator(project));
		movesGenerators.add(new BalancingMovesGenerator(project));	
		
		/*movesGenerators.add(new RandomMovesGenerator(project, 0.16));
		movesGenerators.add(new IdleMovesGenerator(project, 0.9));
		movesGenerators.add(new CommFreqMovesGenerator(project, 0.48));
		movesGenerators.add(new BalancingMovesGenerator(project, 0.08));*/	
	}
	...
	private boolean ts(int iteration) {
		...
		List<GenericMove> possibleMoves = new ArrayList<GenericMove>();
		
		int movesApplied = DEFAULT_REINSERT_NUMBER; // number of moves applied in order to obtain one solution
		
		for (TabuSearchMovesGenerator generator : movesGenerators) {
			generator.generateMoves(sCurrent, iteration); // generate a list of non-tabu moves
			possibleMoves.addAll(generator.getPossibleMovesList());
		}
		
		Iterator<GenericMove> iterator = possibleMoves.iterator();
