...
public class TabuSearchGPUPROBCli implements IApplication {
	
	private Configuration configuration;
	private TabuSearchGPUPROB tabuSearch;
	private IProgressMonitor monitor = new NullProgressMonitor();
	
	public static void main(String[] args) {
		ModelsRegister.init();

		TabuSearchGPUPROBCli cliApp = null;

		try {
			cliApp = new TabuSearchGPUPROBCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				tabuSearch = new TabuSearchGPUPROB(project, weighter, simulator);
