...
public class RandomMovesGenerator extends TabuSearchMovesGenerator {
	...
	public RandomMovesGenerator(TraceProject project) {
		super(project);
	}
	
	public RandomMovesGenerator(TraceProject project, double admissionRate) {
	...
	private List<SingleMove> getAllSingleMoves(NetworkPartitioning startPartitioning, int iteration) {
		...
		for (Actor actor : network.getActors()) {
			if (startPartitioning.asPartitionActorsMap().get(startPartitioning.getPartition(actor)).size() > 1) {
				for (String partition : startPartitioning.getPartitions()) {
