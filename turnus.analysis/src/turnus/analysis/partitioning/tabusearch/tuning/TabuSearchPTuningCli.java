 ...
 * 
 */
public class TabuSearchPTuningCli implements IApplication {
	...
	private static int analysisTime = 318; // time in minutes tuned previously
	...
	private static NetworkPartitioning bestPartitioning;
	
	public static void main(String[] args) {	
		ModelsRegister.init();

		TabuSearchPTuningCli cliApp = null;

		try {
			cliApp = new TabuSearchPTuningCli();
			cliApp.parse(args);
		} catch (TurnusException e) {
			return;
		}
		
		cliApp.configuration.setValue(ANALYSIS_TIME, analysisTime);
		/*
		 * Test different values of p for each seed and each configuration.
		 */
		File mappingFile = null;
		for (int i = 2 ; i < 9; i += 2) {
			double p = (double)i/10.0;
			cliApp.configuration.setValue(TABU_P, p);
			for (int s : seeds) {
				...
				try {
					...
					while ((line = br.readLine()) != null) { // check mapping configurations one by one
						...
						File outputMapping = new File(mappingFile.getParent() + "/" + outputFileName + "_" +  seed + "_" + i + "_" + file.getName());
						File xcfFile = changeExtension(outputMapping, TurnusExtensions.NETWORK_PARTITIONING);
						new XmlNetworkPartitioningWriter().write(bestPartitioning, xcfFile);
	...
	private void run() throws TurnusException {
		...
		{ // STEP 2 : Run the analysis
			monitor.subTask("Tuning of p");
			try {
				// tabu search
				TabuSearchPTuningPortioned tabuSearch = new TabuSearchPTuningPortioned(project, weighter, bufferSize);
				...
				if (outputFileName == null)
					outputFileName = "p_random";
				*/
				
				/*TabuSearchMovesGenerator generator = new IdleMovesGenerator(project);
				tabuSearch.setGenerator(generator);
				if (outputFileName == null)
					outputFileName = "p_idle";
				*/
				
				/*TabuSearchMovesGenerator generator = new CommFreqMovesGenerator(project);
				tabuSearch.setGenerator(generator);
				if (outputFileName == null)
					outputFileName = "p_commfreq";
				*/
				
				TabuSearchMovesGenerator generator = new BalancingMovesGenerator(project);
				tabuSearch.setGenerator(generator);
				if (outputFileName == null)
					outputFileName = "p_balancing";
				
				double time = tabuSearch.run();
				double usedP = configuration.getValue(TABU_P);
				
				bestPartitioning = tabuSearch.getSStar();
				
				String newEntry = mapping + ", seed: " + seed + ", p: " + usedP + ", execution time: " + time + "\n";
