 ...
 * 
 */
public class IdleLocalSearchCli implements IApplication {

	private Configuration configuration;
	private IdleLocalSearch search;
	private IProgressMonitor monitor = new NullProgressMonitor();
	
	public static void main(String[] args) {
		ModelsRegister.init();

		IdleLocalSearchCli cliApp = null;

		try {
			cliApp = new IdleLocalSearchCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		monitor.beginTask("Idle local search partitioning analysis", IProgressMonitor.UNKNOWN);
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				search = new IdleLocalSearch(project, weighter, communication, schedWeight, bufferSize, configuration.getValue(RELEASE_BUFFERS_AFTER_PROCESSING));
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				Logger.info("Idle local search partitioning report stored in \"%s\"", reportFile);
