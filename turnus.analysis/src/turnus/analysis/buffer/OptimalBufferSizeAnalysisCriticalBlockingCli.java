 ...
 */
public class OptimalBufferSizeAnalysisCriticalBlockingCli implements IApplication{
	
	private Configuration configuration;

	private IProgressMonitor monitor = new NullProgressMonitor();

	private OptimalBufferSizeAnalysisCriticalBlocking analysis;

	
	public static void main(String[] args) {
		ModelsRegister.init();

		OptimalBufferSizeAnalysisCriticalBlockingCli cliApp = null;

		try {
			cliApp = new OptimalBufferSizeAnalysisCriticalBlockingCli();
			cliApp.parse(args);
	...
	private void run() throws TurnusException {
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new OptimalBufferSizeAnalysisCriticalBlocking(project);
