 ...
 *
 */
public class BoundedBufferAnalysisCli implements IApplication {

	public static void main(String[] args) {
		ModelsRegister.init();

		BoundedBufferAnalysisCli cliApp = null;

		try {
			cliApp = new BoundedBufferAnalysisCli();
			cliApp.parse(args);
	...
	private BoundedBufferAnalysis analysis;
	...
	private void parse(String[] args) throws TurnusException {
		CliParser cliParser = new CliParser()//
				...
				.setOption(ANALYSIS_BUFFER_BIT_ACCURATE, false);

		configuration = cliParser.parse(args);
	}

	private void run() throws TurnusException {
		monitor.beginTask("Bounded buffer size analysis", IProgressMonitor.UNKNOWN);
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new BoundedBufferAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				Logger.info("Bounded buffer size report stored in \"%s\"", reportFile);
				...
				Logger.info("Bounded buffer size configuration stored in \"%s\"", bxdfFile);
