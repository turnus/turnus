 ...
 *
 */
public class ActionsMarkowModelAnalysisCli implements IApplication {

	private Configuration configuration;
	private IProgressMonitor monitor = new NullProgressMonitor();
	private ActionsMarkowModelAnalysis analysis;
	...
	private void run() throws TurnusException {
		monitor.beginTask("Trace to actions markow model network analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		MarkowModelTraceReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				...
				analysis = new ActionsMarkowModelAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath, TurnusExtensions.TRACE_ACTIONS_MARKOV_ANALYSIS);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Trace actions markov analysis report stored in \"%s\"", reportFile);
	...
	public static void main(String[] args) {
		...
		ActionsMarkowModelAnalysisCli cliApp = null;

		try {
			cliApp = new ActionsMarkowModelAnalysisCli();
			cliApp.parse(args);
		} catch (TurnusException e) {
			return;
		}
