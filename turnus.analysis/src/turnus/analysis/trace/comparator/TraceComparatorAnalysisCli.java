 ...
 *
 */
public class TraceComparatorAnalysisCli implements IApplication {

	private Configuration configuration;
	private IProgressMonitor monitor = new NullProgressMonitor();
	private TraceComparatorAnalysis analysis;
	...
	private void run() throws TurnusException {
		monitor.beginTask("Trace comparator analysis", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		TraceComparatorReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				...
				analysis = new TraceComparatorAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath, TurnusExtensions.TRACE_COMPARATOR_ANALYSIS);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Trace comparator anylsis report stored in \"%s\"", reportFile);
	...
	public static void main(String[] args) {
		...
		TraceComparatorAnalysisCli cliApp = null;

		try {
			cliApp = new TraceComparatorAnalysisCli();
			cliApp.parse(args);
		} catch (TurnusException e) {
			return;
		}
	...
	private void parse(String[] args) throws TurnusException {
		CliParser cliParser = new CliParser()//
				...
				.setOption(TRACE_FILES_SET, true)//
