...
@Accessors class Prefix {
	...
	def boolean canSubtract(Transition t) {
		for (i : 0 ..< t.actions.size) {
			if (t.actions.size > full.combined_size ||
				full.get(full.combined_size - t.actions.size + i) != t.actions.get(i)) {
				return false;
			}
		}
		return true;
	...
	def Prefix subtract(Transition t) {

		for (i : 0 ..< t.actions.size) {
			if (t.actions.size > full.combined_size ||
				full.get(full.combined_size - t.actions.size + i) != t.actions.get(i)) {
				throw new ArithmeticException(
					"Can't subtract the transition from this prefix, since the prefixes tail doesn't match the transition");
			}
		}
	...
	def boolean hasPrefix(Prefix o) {
