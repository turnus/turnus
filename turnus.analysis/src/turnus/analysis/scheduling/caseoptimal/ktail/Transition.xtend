...
@Accessors class Transition {
	...
	new(InferenceState source, InferenceState goal, Integer action) {
		this.goalState = goal;
		this.sourceState = source;
		this.actions.add(action);
	}

	new(InferenceState source, InferenceState goal, Iterable<Integer> actions) {
		this.goalState = goal;
		this.sourceState = source;
		this.actions.addAll(actions);
	}

	def static combine(Transition t1, Transition t2) {
		...
		if (!newTran.connects(t1.sourceState, t2.goalState)) {
			throw new NotImplementedException(
				"The transition formed by combining these two transitions would not actually be able to connect the two states");
		}
		return newTran;
	}
	...
	override boolean equals(Object o) {
		if (o !== null && o instanceof Transition) {
			...
			if (this.goalState == ot.goalState && this.sourceState == ot.sourceState &&
				Arrays.equals(this.actions.toArray, ot.actions.toArray())) {
				return true;
			}
		}
	...
	def boolean connects(Prefix p1, Prefix p2) {
		return connects(p1, p2, true);
	}

	def boolean connects(Prefix p1, Prefix p2, boolean directed) {
		val p1_extended = p1.full.extend(actions);
		return p1_extended.equals(p2.full) || (!directed && p2.full.extend(actions).equals(p1.full));
	}

	def boolean connects(InferenceState s1, InferenceState s2) {
		for (p1 : s1.prefixes) {
			for (p2 : s2.prefixes) {
				if (connects(p1, p2)) {
					return true;
