...
@Accessors class KTailStateFuser {
	...
	def fuseOneToMany(InferenceState s, Iterator<InferenceState> it) {
		...
		for (t : s.outgoings) {
			...
			for (td : debug_transitions_done) {
				Logger.info("Transition %d->%d has %d actions after adding %d->%d", td.sourceState.id, td.goalState.id,
					td.actions.size, t.sourceState.id, t.goalState.id);
			}
		}
		Logger.info("Removing state %d, from fanout chain %d->%d->%s", s.id, pred.id, s.id,
			Arrays.toString(fanout.toArray));
	...
	def fuseManyToOne(InferenceState s, Iterator<InferenceState> it) {
		...
		for (t : s.incomings) {
			...
			suc.incomings.add(newTran);
			fanin.add(pred.id);
			Logger.info("Removing state %d, from fanin chain %s->%d->%d", s.id, Arrays.toString(fanin.toArray), s.id,
				suc.id);
			debug_transitions_done.add(newTran);
			for (td : debug_transitions_done) {
				Logger.info("Transition %d->%d has %d actions after adding %d->%d", td.sourceState.id, td.goalState.id,
