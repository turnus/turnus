...
class KTailOptimizer implements Optimizer {
	...
	def findStates() {
					...
					if (p1Class !== null && p2Class !== null) {
						if (p1Class != p2Class) {
							throw new NotImplementedException(
								"Two equivalent states have different equivalence classes!=null, this should never happen")
		...
		for (e : prefixes.entrySet) {
			val prefix = e.key;
			val s = if (e.value === null) {
					nextClass += 1;
					new InferenceState(nextClass - 1)
				} else {
					e.value
				};
			...
			for (p : ps) {
				for (a : id_to_actors.keySet) {
					...
					if (next_state !== null) {
						...
						Logger.debug("Prefix found, generating transistion from %d->%d, with action %d", state.id,
							next_state.id, a);
						...
						Logger.debug("State %d has new incoming transistion %d->%d, now %d incomings", next_state.id,
							transition.sourceState.id, transition.goalState.id, next_state.incomings.size);
						state.outgoings.add(transition);
						Logger.debug("State %d has new outgoing transistion %d->%d, now %d outgoings", state.id,
							transition.sourceState.id, transition.goalState.id, state.outgoings.size);
	...
	def checkStates() {
		for (state : eqClasses.keySet) {
			Logger.info("State %d from eqClaseses has %d incomings and %d outgoings and %d prefixes", state.id,
				state.incomings.size, state.outgoings.size, state.prefixes.size);
		}
		for (state : prefixes.values.toSet) {
			Logger.info("State %d from prefixes.valueset has %d incomings and %d outgoings and %d prefixes", state.id,
				state.incomings.size, state.outgoings.size, state.prefixes.size);
		}
	}
		...
		while (!curState.equals(fsm.terminalState)) {
			...
			for (u : s.varUpdates) {
				switch u.operation.op {
					case ADD: {
						varstates.addTo(u.operation.^var, u.operation.^val)
					}
					case SET: {
						varstates.put(u.operation.^var, u.operation.^val)
					}
				}
			}
			...
			if (transitionCand.size == 1) {
				...
				switch sched {
					case sched instanceof ActorFire: {
						unrolled_actions.add(sched)
					}
					case sched instanceof Sequence: {
						unrolled_actions.addAll((sched as Sequence).actions)
					}
				}
				...
				Logger.
					error('''Have ambigous transitions from state «curState», have «transitionCand.size» candidates , at step«step»
