...
@Data class Leaf extends Node {
	...
	override fuseTransitions(ScheduleInferenceState state) {
		...
		if (ts.size == 1) {
			...
			val newTran = new ScheduleInfereceTransition(t.actions, new MultiCondition, t.sourceState, t.targetState,
				t.vars);
...
@Data class MultiBranch extends Node {
	...
	override flatten() {
		...
		for (j : 0 ..< conditions.size) {
			...
			switch n {
				case n instanceof Leaf: {
					flat_conditions.add(c);
					flat_nodes.add(n);
				}
				,
	...
	override fuseTransitions(ScheduleInferenceState state) {
		...
		if (state.transitions.size >= conditions.size) {
			...
			throw new KtailConditionFuseException(
				"Expected the number if unique state->action->state transitions to equal the number of branches, not the case.")
		}
	}
...
@Data class Branch extends Node {
	...
	def static void flattenBranch(Node b, BranchCondition bCond, ArrayList<MultiCondition> conditions,
		ArrayList<Node> nodes) {
		switch b {
			case b instanceof Leaf: {
				conditions.add(new MultiCondition(bCond));
				nodes.add(b)
			}
			,
