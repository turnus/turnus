 ...
 *
 */
public class InternalVariablePipeliningAnalysisCli implements IApplication {

	private Configuration configuration;
	private InternalVariablePipeliningAnalysis analysis;
	private IProgressMonitor monitor = new NullProgressMonitor();

	private void run() throws TurnusException {
		monitor.beginTask("Pipelining analysis with internal variables utilization", IProgressMonitor.UNKNOWN);

		TraceProject project = null;
		ActionsVariablePipeliningReport report = null;
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new InternalVariablePipeliningAnalysis(project);
		...
		{ // STEP 3 : Store the results
			...
			try {
				...
				if (configuration.hasValue(OUTPUT_DIRECTORY)) {
					...
					outputPath = createOutputDirectory("pipelining", configuration);
				}

				File reportFile = createFileWithTimeStamp(outputPath,
						TurnusExtensions.ACTION_VARIABLE_PIPELINING_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Action variable pipelining analysis report stored in \"%s\"", reportFile);
	...
	public static void main(String[] args) {
		...
		InternalVariablePipeliningAnalysisCli cliApp = null;

		try {
			cliApp = new InternalVariablePipeliningAnalysisCli();
			cliApp.parse(args);
	...
	private void parse(String[] args) throws TurnusException {
		CliParser cliParser = new CliParser()//
				...
				.setOption(CONFIG_MAX_THREADS, false);
