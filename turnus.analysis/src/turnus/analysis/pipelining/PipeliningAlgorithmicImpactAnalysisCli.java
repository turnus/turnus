 ...
 *
 */
public class PipeliningAlgorithmicImpactAnalysisCli implements IApplication {

	private Configuration configuration;

	private IProgressMonitor monitor = new NullProgressMonitor();

	private PipeliningAlgorithmicImpactAnalysis analysis;

	public static void main(String[] args) {
		ModelsRegister.init();

		PipeliningAlgorithmicImpactAnalysisCli cliApp = null;

		try {
			cliApp = new PipeliningAlgorithmicImpactAnalysisCli();
			cliApp.parse(args);
	...
	private void run() throws Exception {
		...
		{ // STEP 2 : Run the analysis
			...
			try {
				analysis = new PipeliningAlgorithmicImpactAnalysis(project);
				analysis.setConfiguration(configuration);
				report = analysis.run();
				// TODO Logger.infoRaw(report.toString());
			} catch (Exception e) {
		...
		{// STEP 3 : Store the results
			...
			try {
				...
				File reportFile = createFileWithTimeStamp(outputPath,
						TurnusExtensions.PIPELINING_IMPACT_ANALYSIS_REPORT);
				EcoreUtils.storeEObject(report, project.getResourceSet(), reportFile);
				Logger.info("Pipelining algorithmic impact analysis report stored in \"%s\"", reportFile);
